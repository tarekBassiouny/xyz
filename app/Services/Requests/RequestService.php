...
{
    private const ENROLLMENT_STATUS_PENDING = 3;
    ...
    public function createEnrollmentRequest(
        ...
        $pending = Enrollment::where('user_id', $student->id)
            ->where('course_id', $course->id)
            ->where('status', self::ENROLLMENT_STATUS_PENDING)
            ->whereNull('deleted_at')
            ->exists();
        ...
        Enrollment::create([
            ...
            'course_id' => $course->id,
            'center_id' => $course->center_id,
            'status' => self::ENROLLMENT_STATUS_PENDING,
    ...
    public function createDeviceChangeRequest(User $student, ?string $reason): void
    {
        ...
        DeviceChangeRequest::create([
            ...
            'new_device_id' => $active->device_id,
            'new_model' => $active->model ?? 'Unknown',
            'new_os_version' => $active->os_version ?? 'unknown',
