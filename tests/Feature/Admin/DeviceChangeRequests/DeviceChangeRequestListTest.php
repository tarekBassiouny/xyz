...
it('applies filters and pagination', function (): void {
    ...
    $response = $this->actingAs($super, 'admin')->getJson('/api/v1/admin/device-change-requests?status=2&user_id='.$user->id.'&date_from='.now()->subDays(3)->toDateString().'&per_page=1');
