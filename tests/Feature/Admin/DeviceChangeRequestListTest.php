...
it('allows super admin to list requests across centers', function (): void {
    ...
    $response = $this->actingAs($super, 'admin')->getJson('/api/v1/admin/device-change-requests', $this->adminHeaders());
...
it('applies filters and pagination', function (): void {
    ...
    $response = $this->actingAs($super, 'admin')->getJson('/api/v1/admin/device-change-requests?status='.DeviceChangeRequest::STATUS_APPROVED->value.'&user_id='.$user->id.'&date_from='.now()->subDays(3)->toDateString().'&per_page=1', $this->adminHeaders());
